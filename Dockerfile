...
# REVISIÓN: 2026.10.20 (Vite Build Fix: Force Dev Deps)
...
# Instalación COMPLETA (incluye devDependencies para tener acceso a 'vite')
# Forzamos NODE_ENV=development para evitar que se omitan devDependencies si el arg global es production
RUN NODE_ENV=development npm install --legacy-peer-deps && \
    NODE_ENV=development npm install zod@3.24.1 react-hook-form@7.54.1 @hookform/resolvers@3.9.1 redis@4.6.13 jsonwebtoken@9.0.2 --legacy-peer-deps

# Inyección total del código fuente
COPY . .

# Generación del bundle de alta disponibilidad (Ahora sí encontrará 'vite')
RUN npm run build
...
# Transferencia de binarios validados desde el Builder
COPY --from=builder /app/dist ./dist
COPY --from=builder /app/server.js ./
COPY --from=builder /app/package*.json ./

# Instalación de dependencias LIGERAS (Solo producción) para el servidor
RUN npm install --omit=dev --legacy-peer-deps && \
    npm install redis@4.6.13 jsonwebtoken@9.0.2 --no-save && \

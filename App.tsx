...
const ProtectedRoute = ({ children, allowedRoles }: { children?: React.ReactNode, allowedRoles?: Role[] }) => {
  ...
  React.useEffect(() => {
    ...
    if (allowedRoles && user) {
      const hasPermission = allowedRoles.includes(user.role) || user.role === 'GOD_MODE';
  ...
  if (allowedRoles && user && !allowedRoles.includes(user.role) && user.role !== 'GOD_MODE') {
...
const Navbar = ({ maintenanceMode, settings }: { maintenanceMode: boolean, settings?: any }) => {
  ...
  if (normalizedPath === '/' || normalizedPath === '/book' || normalizedPath === '/login') return null;
    ...
    <nav className={`sticky top-0 z-50 w-full backdrop-blur-2xl border-b h-20 shadow-2xl transition-all ${theme === 'dark' ? 'bg-black/90 border-white/10' : 'bg-white/90 border-slate-200'
      ...
      <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link to={user?.role === 'CLIENT' ? '/client-portal' : user?.role === 'STAFF' ? '/professional-dashboard' : user?.role === 'GOD_MODE' ? '/nexus' : '/admin'}>
          ...
          {user && (user.role === 'ADMIN' || user.role === 'GOD_MODE') && (
              ...
              <div className="relative group/catalog">
                ...
                <div className="absolute top-full left-0 pt-2 opacity-0 translate-y-2 pointer-events-none group-hover/catalog:opacity-100 group-hover/catalog:translate-y-0 group-hover/catalog:pointer-events-auto transition-all duration-300">
                  <div className="glass-card w-64 rounded-[2rem] p-3 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] bg-black/95 backdrop-blur-xl">
                    ...
                    <Link to="/marketing" className="flex items-center gap-3 px-4 py-3.5 text-[9px] font-black uppercase tracking-widest text-zinc-300 hover:text-[#D4AF37] hover:bg-white/10 rounded-2xl transition-all"><Megaphone size={14} /> Marketing</Link>
                    <Link to="/leads" className="flex items-center gap-3 px-4 py-3.5 text-[9px] font-black uppercase tracking-widest text-[#D4AF37] bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 rounded-2xl transition-all"><UserPlus size={14} /> Gestión de Leads</Link>
                    <Link to="/maintenance" className="flex items-center gap-3 px-4 py-3.5 text-[9px] font-black uppercase tracking-widest text-emerald-400 bg-emerald-400/10 hover:bg-emerald-400/20 rounded-2xl transition-all"><ShieldCheck size={14} /> Mantenimiento</Link>

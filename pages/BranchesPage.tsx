...
export const BranchesPage: React.FC = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (branch: Branch) => {
      if (editingBranch) {
        const success = await api.updateBranch(branch);
        if (!success) throw new Error("Falla en actualización.");
        return { success: true };
      }
      const { id, ...data } = branch;
      const res = await api.createBranch(data);
      if (!res.success) throw new Error("Falla en integración.");
      return res;
    },
    onSuccess: () => {
    ...
    onError: (e: any) => {
      toast.error(e.message || "Falla en el despliegue del nodo.");
    }
  });

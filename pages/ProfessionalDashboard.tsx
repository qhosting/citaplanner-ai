...
export const ProfessionalDashboard: React.FC = () => {
  ...
  useEffect(() => {
    socketService.on('new-appointment', (newApt: Appointment) => {
      // If the appointment belongs to this professional (or if we want to update global list)
      if (newApt.professionalId === user?.relatedId) {
        queryClient.invalidateQueries({ queryKey: ['pro-appointments', user?.relatedId] });
        toast.info(`Nueva cita agendada: ${newApt.title}`);
      }
    });
  ...
  const handleSyncGoogle = async () => {
    try {
      ...
      const { url } = await api.getCalendarLink(user.relatedId);
  ...
  const copyICal = async () => {
    try {
      if (!user?.relatedId) return;
      const { icalToken } = await api.getCalendarLink(user.relatedId);
      const url = `${window.location.protocol}//${window.location.host}/api/calendar/feed/${icalToken}.ics`;
      await navigator.clipboard.writeText(url);
      toast.success("Enlace iCal copiado. Pégalo en Apple Calendar.");
    } catch (e) {
      toast.error("Error al obtener enlace iCal");
    }
  };
  ...
  if (isLoading) return <div className="h-screen flex items-center justify-center bg-transparent"><Loader2 className="animate-spin text-[#D4AF37]" size={40} /></div>;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 animate-entrance">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
        <div>
          ...
          <p className="text-muted font-bold uppercase tracking-[0.5em] text-[10px] ml-5">Professional Workspace • Aurum Network</p>
        </div>
        <div className="flex items-center gap-6 bg-white/5 px-8 py-4 rounded-[2rem] border border-white/5">
          <div className="text-right">
            <p className="text-[10px] font-black text-muted uppercase tracking-widest">Especialista</p>
            <p className="text-lg font-black text-main">{user?.name}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-8 space-y-12">
          <section>
            <div className="flex justify-between items-center mb-10">
              ...
              <span className="text-[10px] font-black text-muted uppercase tracking-[0.4em]">{todayAppointments.length} Sesiones</span>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                      <div className="flex gap-8 items-center">
                        <div className="text-center min-w-[80px]">
                          <p className="text-2xl font-black text-main">{new Date(apt.startDateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                        ...
                        <div>
                          <h3 className="text-xl md:text-2xl font-black tracking-tight text-main group-hover:text-[#D4AF37] transition-colors">{apt.title}</h3>
                          <div className="flex items-center gap-4 mt-1">
                            <span className="text-xs font-bold text-muted flex items-center gap-1"><User size={14} /> {apt.clientName}</span>
                            <span className="w-1.5 h-1.5 rounded-full bg-[#D4AF37]/30" />
                            <span className="text-[10px] font-black text-muted uppercase tracking-widest">Confirmada</span>
                          </div>
                        </div>
                      </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {upcoming.filter(a => new Date(a.startDateTime).toLocaleDateString() !== todayStr).slice(0, 4).map(apt => (
                <div key={apt.id} className="glass-card p-6 rounded-[2.5rem] flex justify-between items-center group">
                  <div>
                    <p className="text-[9px] font-black text-muted uppercase tracking-widest mb-1">{new Date(apt.startDateTime).toLocaleDateString('es-ES', { weekday: 'short', day: 'numeric', month: 'short' })}</p>
                    <h4 className="font-black text-main group-hover:text-[#D4AF37] transition-colors">{apt.title}</h4>
                  </div>
                  <span className="text-[10px] font-black text-muted">{new Date(apt.startDateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          {/* Summary Stats */}
          <div className="glass-card p-10 rounded-[3.5rem] bg-gradient-to-tr from-luxury-black/5 to-luxury-black border-[#D4AF37]/5">
            <h3 className="text-[10px] font-black uppercase tracking-[0.5em] mb-10 flex items-center gap-3 text-main">
              <Zap className="text-[#D4AF37]" size={20} /> Métricas de Desempeño
            </h3>
            <div className="space-y-8">
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-5xl font-black tracking-tighter text-main">{todayAppointments.filter(a => a.status === 'COMPLETED').length}</p>
                  <p className="text-[9px] font-black text-muted uppercase tracking-widest mt-1">Sesiones Finalizadas Hoy</p>
                </div>
          ...
          <div className="glass-card p-10 rounded-[3.5rem]">
            ...
            <div className="space-y-4">
              <button onClick={handleSyncGoogle} className="w-full flex items-center justify-between p-5 bg-gradient-to-r from-blue-600/20 to-blue-400/10 rounded-2xl border border-blue-500/20 hover:border-blue-500/50 transition-all text-left">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase tracking-widest text-main">Sincronizar Google</span>
                  <span className="text-[8px] font-bold text-blue-400/70 uppercase">Actualización en tiempo real</span>
                </div>
                <CalendarIcon size={16} className="text-blue-400" />
              </button>
              <button onClick={copyICal} className="w-full flex items-center justify-between p-5 bg-gradient-to-r from-slate-600/20 to-slate-400/10 rounded-2xl border border-slate-500/20 hover:border-slate-500/50 transition-all text-left">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase tracking-widest text-main">Enlace iCalendar (Apple)</span>
                  <span className="text-[8px] font-bold text-slate-400/70 uppercase">Copia URL de suscripción</span>
                </div>
              ...
              <button className="w-full flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 hover:border-[#D4AF37]/30 transition-all text-left">
                <span className="text-[10px] font-black uppercase tracking-widest text-main">Ver Inventario Staff</span>
              ...
              <button onClick={enableNotifications} className="w-full flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 hover:border-[#D4AF37]/30 transition-all text-left">
                <span className="text-[10px] font-black uppercase tracking-widest text-main">Activar Alertas (Push)</span>
                <Bell size={16} className="text-[#D4AF37]" />
              </button>
              <button className="w-full flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 hover:border-[#D4AF37]/30 transition-all text-left">
                <span className="text-[10px] font-black uppercase tracking-widest text-main">Protocolos de Lujo</span>
          ...
          <div className="glass-card w-full max-w-2xl rounded-[3.5rem] p-12 border-[#D4AF37]/20 animate-scale-in">
            <div className="flex justify-between items-start mb-10">
              <div>
                ...
                <h2 className="text-3xl md:text-4xl font-black tracking-tighter uppercase text-main">{selectedApt.title}</h2>
                <p className="text-muted font-bold mt-2">Cliente: {selectedApt.clientName}</p>
              </div>
              <button onClick={() => setSelectedApt(null)} className="p-3 bg-white/5 rounded-2xl text-muted hover:text-main"><AlertCircle size={24} /></button>
            </div>

            <div className="space-y-8">
              <div>
                <label className="text-[9px] font-black text-muted uppercase tracking-widest mb-3 block ml-2">Notas Técnicas y Seguimiento (Expediente Clínico)</label>
                <textarea
                  rows={6}
                  value={notes}
                  onChange={e => setNotes(e.target.value)}
                  placeholder="Describe el resultado, pigmentos usados, profundidad y recomendaciones para el cliente..."
                  className="w-full bg-black/40 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-[2.5rem] p-8 text-main outline-none focus:border-[#D4AF37] transition-all resize-none font-medium leading-relaxed"
                />
              </div>
